...
//   const handleGenerate = async () => {
//     // Basic validation: ensure a job title is entered
//     if (!generationInput.job_title_input.trim()) {
//         setError('Please enter a job title to generate a description.');
//         return;
//     }
...
//     setError('');
//     try {
//       // Create a payload with only the job title.
//       // Other fields are sent as empty strings/arrays to satisfy the API contract.
//       const payload = {
//         job_title_input: generationInput.job_title_input,
//         key_responsibilities_input: [], // Send as empty array
//         required_skills_input: [], // Send as empty array
//         company_description_input: "", // Send as empty string
//       };

//       const generatedData = await generateJD(payload);
//        console.log("AI RESPONSE (generatedData):", generatedData);
//       const formattedContent = {
//         ...generatedData,
//         key_responsibilities: generatedData.key_responsibilities.join('\n'),
//         required_qualifications: generatedData.required_qualifications.join('\n'),
...
//         benefits: generatedData.benefits.join('\n'),
//       };
//       setJobData((prev) => ({ ...prev, job_title: generatedData.job_title_generated || generationInput.job_title_input, jd_content: formattedContent }));
//       setSnackbar({ open: true, message: 'JD generated successfully!' });
//     } catch (err) {
//       setError('Failed to generate JD from AI. Check your inputs and try again.');
//       console.error(err);
...
//               placeholder="e.g., Senior Frontend Developer with React"
//               value={generationInput.job_title_input}
//               onChange={handleGenerationInputChange}
//               helperText="The AI will generate a complete job description based on this title."
//             />
//             {/* The other text fields are now hidden from the UI */}
//             <Box>
//               <LoadingButton
//                 variant="contained"
...
const JobForm = ({ jobId , onSaveSuccess }) => {
  const [mode, setMode] = useState(jobId ? 'edit' : 'create');
  const [loading, setLoading] = useState(true);
  const [generating, setGenerating] = useState(false);
  const [saving, setSaving] = useState(false);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let savedJob;
      if (mode === 'create') {
        savedJob = await createJD(payload);
        setSnackbar({ open: true, message: 'Job created successfully!' });
      } else {
        savedJob = await updateJobById(jobId, payload);
        setSnackbar({ open: true, message: 'Job updated successfully!' });
      }
